...
const ContractPage = () => {
    ...
    const generateTableBody = (items) => {
        return items.map((contract, index) => (
            <tr key={index}>
                <td className='space-x-1'>{contract.firstName} {contract.lastName}</td>
        ...
        <div>
            <div >
                ...
                <div className="flex justify-between items-end mb-2">
                    ...
                    />
                    <span>Showing {filteredData.length > 0 ? indexOfFirstItem + 1 : 0} - {Math.min(indexOfLastItem, filteredData.length)} of {filteredData.length} results</span>
                    <Search searchQuery={searchQuery} onSearch={handleSearch} />
                </div>

                <div className="overflow-auto w-full ">
                    <TableCustom headers={headers} body={generateTableBody(currentItems)} />
                </div>
